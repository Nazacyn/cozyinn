    ...
    (async () => {
      ...
      try {
        ...
        if (fetched?.stripePublishableKey) {
          config.stripePublishableKey =
            fetched.stripePublishableKey;
        }
      } catch (e) {
        console.log(
          "[Voiceflow] Using fallback configuration."
        );
      }
      ...
      script.onload = () => {
        if (!window.voiceflow?.chat) {
          console.error(
            "Voiceflow widget failed to initialize"
          );
          return;
        }
        ...
        const StripePaymentExtension = {
          ...
          render: async ({ trace, element }: any) => {
            try {
              ...
              const clientSecret =
                trace.payload?.client_secret;

              if (!clientSecret) {
                throw new Error(
                  "Missing PaymentIntent client_secret"
                );
              }
              ...
              container.className =
                "vf-stripe-payment p-4 rounded-xl bg-white shadow border border-gray-100 my-2";
              ...
              payBtn.className =
                "mt-3 w-full rounded-full bg-[#0070f3] text-white px-4 py-2.5 text-sm font-semibold hover:opacity-90 transition";
